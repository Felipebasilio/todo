...
interface InputCheckboxProps extends Omit<ComponentProps<"input">, "disabled" | "size">, VariantProps<typeof inputCheckboxVariants> {
  loading?: boolean
}
...
export default function InputCheckbox({ variant, size, disabled, className, loading, ...props }: InputCheckboxProps) {
  ...
  if (loading) {
    return <Skeleton rounded="sm" className={inputCheckboxVariants({ variant: "none", size, className })} />
  }
